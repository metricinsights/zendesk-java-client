...
public class Zendesk implements Closeable {
    ...
    public Iterable<Ticket> getTicketsIncrementally(Date startTime) {
        return new PagedIterable<>(
                tmpl("/incremental/tickets.json{?start_time}&include=metric_sets,dates")
    ...
    public Iterable<Ticket> getTicketsByView(long viewId) {
        return new PagedIterable<>(tmpl("/views/{viewId}/tickets.json?include=metric_sets,dates").set("viewId", viewId),
                handleList(Ticket.class, "tickets"));
    }
    ...
    */
    ...
    public Zendesk build() {
      if (token != null) {
        return new Zendesk(client, url, username + "/token", token, headers, retry, cbpPageSize);
      } else if (oauthToken != null) {
        return new Zendesk(client, url, oauthToken, headers, retry, cbpPageSize);
      }
      return new Zendesk(client, url, username, password, headers, retry, cbpPageSize);
    }
  }
}
